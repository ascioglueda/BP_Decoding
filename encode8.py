...
class Node(DawnSimVis.BaseNode):
    ...
    def start_decoding(self):
        ...
        if self.id == SOURCE and self.received_hops:
            total_hops = sum(self.received_hops)
            self.log(f"Root'a ulaşan paketlerin toplam hops sayısı: {total_hops}")
    ...
    def on_receive(self, pck):
        ...
        if self.currstate == 'XPLORING':
            ...
            if self.id == SOURCE:
                if sender_id != self.id:  # Kendi gönderdiği paketi alma
                # Paket root'a ulaştı, decode için sakla
                    ...
                    self.received_hops.append(pck['hop_count'])
            else:
                # Root değilse parent'a ilet
                if self.parent is not None:
                    new_pck = pck.copy()
                    new_pck['hop_count'] = pck.get('hop_count', 0) + 1
                    self.send(self.parent, new_pck)
                    self.log(f"Node {self.id} forwarded packet to parent {self.parent} (hop: {new_pck['hop_count']})")
    def finish(self):
        pass
